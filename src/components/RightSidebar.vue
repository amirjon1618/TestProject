        ...
        <div class="new-post rounded-xl w-full bg-white px-8 py-5 mb-5" v-for="addPost in post" :key="addPost.id" :id="addPost.id">
            ...
            <div class="flex pt-2 justify-between">
             ...
             <div class="post-like hover:cursor-pointer flex">
                 <svg width="24px" height="24px" @click="clickLike" style="display: none;" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="iconify iconify--twemoji" preserveAspectRatio="xMidYMid meet"><path fill="#DD2E44" class="pointer-events-none" d="M35.885 11.833c0-5.45-4.418-9.868-9.867-9.868c-3.308 0-6.227 1.633-8.018 4.129c-1.791-2.496-4.71-4.129-8.017-4.129c-5.45 0-9.868 4.417-9.868 9.868c0 .772.098 1.52.266 2.241C1.751 22.587 11.216 31.568 18 34.034c6.783-2.466 16.249-11.447 17.617-19.959c.17-.721.268-1.469.268-2.242z"></path></svg>
                <svg width="24px" height="24px" @click="clickLike" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="iconify iconify--twemoji" preserveAspectRatio="xMidYMid meet"><path fill="#92929D" class="pointer-events-none" d="M35.885 11.833c0-5.45-4.418-9.868-9.867-9.868c-3.308 0-6.227 1.633-8.018 4.129c-1.791-2.496-4.71-4.129-8.017-4.129c-5.45 0-9.868 4.417-9.868 9.868c0 .772.098 1.52.266 2.241C1.751 22.587 11.216 31.568 18 34.034c6.783-2.466 16.249-11.447 17.617-19.959c.17-.721.268-1.469.268-2.242z"></path></svg><span class="flex items-center px-1">{{addPost.count}}</span>
            </div>
            </div>
        </div>
  ...
  methods:{
      addPost(){
        ...
        clickLike({target}){   
            ...
            target.parentNode.childNodes[2].textContent = '1';
            }
            else{
                target.parentNode.childNodes[1].style.display = "block";
                target.parentNode.childNodes[0].style.display = "none";
                target.parentNode.childNodes[2].textContent = '0';
            }
           // target.parentNode.childNodes[1].textContent = '';
                       /* if(this.count %2 === 0){
                this.count++;
            }else this.count--;*/
        }
